...
function TaskDetail({ user }) {
  ...
  const loadTaskDetails = async () => {
    try {
      ...
      // Load messages if agreement exists
      if (taskAgreement) {
        try {
          const messagesRes = await getTaskMessages(id);
  ...
  const handleSendMessage = async (e) => {
    ...
    setSuccess('');
    try {
      await sendTaskMessage(id, newMessage);
      setNewMessage('');
              ...
              {messages.map((msg) => (
                <div
                  ...
                  style={{
                    padding: '10px',
                    margin: '10px 0',
                    backgroundColor: msg.sender_id === user.id ? '#e3f2fd' : '#fff',
                    borderRadius: '8px',
                    marginLeft: msg.sender_id === user.id ? '20%' : '0',
                    marginRight: msg.sender_id === user.id ? '0' : '20%',
                    boxShadow: '0 1px 2px rgba(0,0,0,0.1)'
                  }}
                >
                  <div style={{ fontSize: '12px', color: '#666', marginBottom: '5px' }}>
                    <strong>{msg.sender_id === user.id ? 'You' : (user.role === 'customer' ? 'Tasker' : 'Customer')}</strong>
                    {' • '}
